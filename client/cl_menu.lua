...
RegisterNetEvent('phabrill_vetement:loadPlayerClothe')
AddEventHandler('phabrill_vetement:loadPlayerClothe', function(data)
    PlayerClothe = data
end)

RegisterNetEvent('phabrill_vetement:Notification')
AddEventHandler('phabrill_vetement:Notification', function(text)
    ESX.ShowNotification(""..text.."")
end)

RegisterNetEvent('phabrill_vetement:refreshClothe')
AddEventHandler('phabrill_vetement:refreshClothe', function(data)
    PlayerClothe = data
end)
...
AddEventHandler('onClientResourceStart', function (resourceName)
    ...
    TriggerServerEvent('phabrill_vetement:loadClothe')
end)
...
function RageUI.PoolMenus:phabrill()
    ...
    magasin:IsVisible(function(Items)
        Items:AddButton("Payer", nil, { IsDisabled = false, RightLabel = "~g~" .. phabrillVetement.Prix .. "$" }, function(onSelected)
            if (onSelected) then
                TriggerServerEvent('phabrill_vetement:achetervetement', phabrillVetement.Prix)
            end
        end)
        ------------------- T-shirt -------------------
    ...
    tenu:IsVisible(function(Items)
        Menuphabrill_vetement()
    end, function()
    end)
end

function Menuphabrill_vetement()
            ...
            Items:AddList(v.name, utils, index.utils, nil, { IsDisabled = false }, function(Index, onSelected, onListChange)
                ...
                if (onSelected) then
                    if (Index) == 1 then                  
                        LoadClothe(v.clothe)
                    elseif (Index) == 2 then 
                        TriggerServerEvent('phabrill_vetement:dropClothe', v.name)
                    elseif (Index) == 3 then
                        local ClosestPlayer, ClosestDistance = GetClosestPlayer()
                        if ClosestPlayer ~= -1 and ClosestDistance <= 3.0 then 
                            TriggerServerEvent('phabrill_vetement:giveClothe', v.name, GetPlayerServerId(ClosestPlayer))
                        else
...
Citizen.CreateThread(function()
	while true do
		...
		for k, v in pairs(phabrillVetement.Position) do
...
function LoadClothe(clothe)
    Citizen.CreateThread(function()
        ...
        for _,v in pairs(clothe) do
            ESX.TriggerServerCallback('phabrill_vetement:getPlayerSkin', function(cb)
                comp = cb
...
RegisterNetEvent("phabrill_vetement:yesmoney")
AddEventHandler(("phabrill_vetement:yesmoney"), function()
    ...
    if name == nil or name == '' then 
        ...
        TriggerServerEvent('phabrill_vetement:saveClothe', name, plyClothe)
    end
end)

RegisterNetEvent("phabrill_vetement:nomoney")
AddEventHandler(("phabrill_vetement:nomoney"), function()
    ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
        TriggerEvent('skinchanger:loadSkin', skin)
    end)
end)